# Form implementation generated from reading ui file 'RegistrationDialog.ui'
#
# Created by: PyQt6 UI code generator 6.10.2
#
# WARNING: Any manual changes made to this file will be lost when pyuic6 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt6 import QtCore, QtGui, QtWidgets


class Ui_RegistrationDialog(object):
    def setupUi(self, RegistrationDialog):
        RegistrationDialog.setObjectName("RegistrationDialog")
        RegistrationDialog.resize(516, 334)
        self.verticalLayout = QtWidgets.QVBoxLayout(RegistrationDialog)
        self.verticalLayout.setObjectName("verticalLayout")
        self.formLayout = QtWidgets.QFormLayout()
        self.formLayout.setObjectName("formLayout")
        self.label = QtWidgets.QLabel(parent=RegistrationDialog)
        font = QtGui.QFont()
        font.setPointSize(15)
        self.label.setFont(font)
        self.label.setObjectName("label")
        self.formLayout.setWidget(0, QtWidgets.QFormLayout.ItemRole.LabelRole, self.label)
        self.attendeeCombo = QtWidgets.QComboBox(parent=RegistrationDialog)
        self.attendeeCombo.setStyleSheet("QComboBox { border: 1px solid #6f6f6f; padding: 2px; color: black; }")
        self.attendeeCombo.setObjectName("attendeeCombo")
        self.formLayout.setWidget(0, QtWidgets.QFormLayout.ItemRole.FieldRole, self.attendeeCombo)
        self.verticalLayout.addLayout(self.formLayout)
        spacerItem = QtWidgets.QSpacerItem(20, 40, QtWidgets.QSizePolicy.Policy.Minimum, QtWidgets.QSizePolicy.Policy.Expanding)
        self.verticalLayout.addItem(spacerItem)
        self.horizontalLayout = QtWidgets.QHBoxLayout()
        self.horizontalLayout.setObjectName("horizontalLayout")
        spacerItem1 = QtWidgets.QSpacerItem(40, 20, QtWidgets.QSizePolicy.Policy.Expanding, QtWidgets.QSizePolicy.Policy.Minimum)
        self.horizontalLayout.addItem(spacerItem1)
        self.btnRegister = QtWidgets.QPushButton(parent=RegistrationDialog)
        self.btnRegister.setObjectName("btnRegister")
        self.horizontalLayout.addWidget(self.btnRegister)
        self.btnCancel = QtWidgets.QPushButton(parent=RegistrationDialog)
        self.btnCancel.setObjectName("btnCancel")
        self.horizontalLayout.addWidget(self.btnCancel)
        self.verticalLayout.addLayout(self.horizontalLayout)

        self.retranslateUi(RegistrationDialog)
        self.btnCancel.clicked.connect(RegistrationDialog.reject) # type: ignore
        QtCore.QMetaObject.connectSlotsByName(RegistrationDialog)

    def retranslateUi(self, RegistrationDialog):
        _translate = QtCore.QCoreApplication.translate
        RegistrationDialog.setWindowTitle(_translate("RegistrationDialog", "Register Attendee"))
        RegistrationDialog.setStyleSheet(_translate("RegistrationDialog", "background-color: #f1f5f9;"))
        self.label.setStyleSheet(_translate("RegistrationDialog", "color: rgb(0, 0, 0);"))
        self.label.setText(_translate("RegistrationDialog", "Select Attendee:"))
        self.btnRegister.setStyleSheet(_translate("RegistrationDialog", "padding: 8px 15px; color: rgb(255, 255, 255); background-color: #3b82f6;"))
        self.btnRegister.setText(_translate("RegistrationDialog", "💾 Register"))
        self.btnCancel.setStyleSheet(_translate("RegistrationDialog", "padding: 8px 15px; color: rgb(255, 255, 255); background-color: #ef4444;"))
        self.btnCancel.setText(_translate("RegistrationDialog", "❌ Cancel"))
