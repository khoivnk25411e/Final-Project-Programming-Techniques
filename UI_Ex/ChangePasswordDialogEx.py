...
class ChangePasswordDialogEx(Ui_ChangePasswordDialog):
    ...
    def _init_form(self):
        if self.current_user:
            self.lblUser.setText(f"Account: {self.current_user.FullName} ({self.current_user.UserName})")
    ...
    def setupSignalAndSlot(self):
        ...
        self.btnToggleOld.clicked.connect(lambda: self._toggle_echo(self.lineEditOldPwd))
        self.btnToggleNew.clicked.connect(lambda: self._toggle_echo(self.lineEditNewPwd))
    ...
    def process_change(self):
        ...
        if not self.skip_old_password:
            ...
            if not old_pwd:
                QMessageBox.warning(self.dialog, "Error", "Please enter your current password!")
                return
            if old_pwd != self.current_user.Password:
                QMessageBox.warning(self.dialog, "Incorrect", "The current password is incorrect!")
                return

        if len(new_pwd) < 6:
            QMessageBox.warning(self.dialog, "Error", "The new password must be at least 6 characters long!")
            return

        if new_pwd != confirm:
            QMessageBox.warning(self.dialog, "Error", "Password confirmation does not match!")
            return

        if not self.skip_old_password and new_pwd == self.current_user.Password:
            QMessageBox.warning(self.dialog, "Error", "The new password must be different from the old password!")
            return
        ...
        QMessageBox.information(self.dialog, "Success", "Password changed successfully!")
        self.dialog.accept()
